---
name: uat
description: Run the Robin User Acceptance Test suite end-to-end against the M2 single-user stack. Boots the server, seeds the OpenRouter key, signs in as the env-seeded user, runs the canonical ingest acceptance test (Sarah example), validates the failure-path self-healing loop, and writes a structured run report. Asserts deterministic API contracts; observes (does not fail on) probabilistic LLM output.
---

UAT boots Robin from a clean DB, runs the canonical M2 ingest loop end-to-end, and
writes a machine-readable run report. It is **destructive** — Phase 1 step 5 wipes
and re-pushes the schema unconditionally. Do not point it at a database you care
about.

Two classes of result:

- **Assert** — deterministic API contracts. Boot, auth, dedup, state-machine
  transitions, schema shape, failure-path audit columns, dormant-marker pin.
  Failures here exit non-zero and fail the run.
- **Observe** — probabilistic LLM output. Fragment counts, entity names, embedding
  presence, mention/wiki edge counts, end-to-end latency. Recorded in the report
  but never fail the run. M2's retro calls out that greenfield wiki classification
  is a no-op, so wiki-edge counts are expected to be zero and not asserted.
...
## Phase 7 — Dormant wiki regen pin

### Step 16 -- Wiki regen returns dormant marker
...
if [ "$WIKI_RESP" = "201" ] || [ "$WIKI_RESP" = "200" ]; then
  ...
  halt "cannot test dormant pin without a wiki id"
fi

# Pin the dormant shape: 503 { error: "Wiki regen disabled in M2 — will be restored in M3" }
REGEN_CODE=$(curl -s -o /tmp/uat-regen.json -w '%{http_code}' \
  -X POST "${BASE_URL}/wikis/${WIKI_ID}/regenerate" \
  -b "$COOKIE_JAR")
REGEN_ERR=$(jq -r '.error // empty' /tmp/uat-regen.json)

if [ "$REGEN_CODE" = "503" ] && echo "$REGEN_ERR" | grep -q 'disabled in M2'; then
  assert "wiki.regen_dormant_pin" "pass" "regen pinned: 503 '${REGEN_ERR}'"
else
  cat /tmp/uat-regen.json
  assert "wiki.regen_dormant_pin" "fail" "expected 503 + 'disabled in M2', got ${REGEN_CODE}: ${REGEN_ERR}"
fi
# Tripwire intent: when M3 lands wiki regen, this assert flips and the test author
# must retire the pin.
```
...

{
  echo "# UAT run ${RUN_ID}"
  echo
  echo "- Milestone: M2 (shipped 2026-04-11)"
